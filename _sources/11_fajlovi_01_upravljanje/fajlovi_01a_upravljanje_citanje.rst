...
- Параметар ``maska`` је опис коме треба да одговарају имена постојећих фајлова и фолдера. На пример, маска ``"*.txt"`` описује фајлове са екстензијом ``.txt``, маска ``"a*.*"`` описује фајлове и фолдере чије име почиње на слово `a` итд.
...
- ``DirectoryInfo Directory.GetParent(string putanja)`` - враћа структуру са информацијама о родитељском фолдеру. Структуром ``DirectoryInfo`` се нећемо овде бавити, осим што ћемо је конвертовати у стринг методом ``.ToString()`` да бисмо добили путању до родитељског фолдера.
